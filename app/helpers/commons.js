...
function pgModelErrorResponse(err) {
  console.log(err)
	let pgErrorCode = {
		'0': 'No data returned from the query.',
		'42703': 'Column undefined.',
		'42P01': 'Table undefined.',
		'42601': 'Syntax error',
		'23502': colName => `${colName} should not be empty.` 
	}
  let error = {}
	if(pgErrorCode[err.code]) {
		error['error'] = pgErrorCode[err.code]
    error['status'] = 404
	}else{
    error['error'] = pgErrorCode[err.code] || 'Not found error code'
  }
	throw error
}
